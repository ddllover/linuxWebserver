...
* auto 推断用法，为什么c++里面会有专门的const迭代器，但大部分算法只支持非const迭代器
，另一方面在自己实现的模板类里面理解为何会有迭代器这种东西，而不是直接把元素给你/指针给你
编译优化问题 -O2,正则表达式很慢？
* debug 困难，stl dgb打印问题
* tcp粘包问题，http请求主体和下一个请求行之间是没有任何其他方式分辨的，必须使用content-length请求头字段
* 线程安全的问题，提一下上次提过的LRU策略的缓冲区，第一次设计锁颗粒太小了，导致并没有实现实际想要的互斥操作。
另一部分是和系统相关的，超时强制关闭连接时，此时所有的东西不需要去保证线程安全，因为那些操作已经没有意义了

### 谈一下自己的编译器

词法分析的基础是正则表达式，有限状态机
词法分析，之后每个词提取出来，词内还有词，都是词，使用一个抽象类词作为基本的接口，其他有点含义的继承类词
我们就可以使用统一的类型去管理这些对象了
可以利用这种方式去构建一个语法分析树

语法分析：就是词与词直接逻辑关系，比如a=b+c 有三个五个词a = b + c 他代表的是语义是？加法指令/偏移量，加载指令
再比如for(a;b;c) a是只运行一次，b是逻辑判断，c循环内的语句，循环通过跳转指令
再比如全局的关系，有一个全局变量a，一个函数/作用域也有个a，这个a代表哪个a

构建汇编指令，最关键的是函数，每次出现函数，对于每个函数我们要计算需要分配栈的大小，需要传参时，寄存器的变化
